...
!(function ($, window) {
    ...
    var AjaxBootstrapSelectList = function (plugin) {
        ...
        plugin.$element.find('option').each(function () {
            ...
            var data = {
                ...
                data: $option.data() || {},
                preserved: isSelected,
                selected: isSelected
            };
            initial_options.push(data);
            if (isSelected) {
                shouldRebuid = true;
        ...
        if (shouldRebuid) {
            this.replaceOptions(this.cacheGet(''));
        }
    ...
    AjaxBootstrapSelectList.prototype.destroy = function () {
        this.replaceOptions();
        this.plugin.list.setStatus(this.plugin.t('statusInitialized'));
    ...
    AjaxBootstrapSelectRequest.prototype.complete = function (jqXHR, status) {
        ...
        if (status !== 'abort') {
            ...
            this.plugin.$element.trigger('complete.request');
        }
    };
