define(function (require) {
    ...
    var BaseModel = require('models/consumer.medicaid.number.model'),
        BaseView = require('views/base.contact.search.view');

    return BaseView.extend({

        container: '<div id="medicaid-number-view" class="modal fade" tabindex="-1" role="dialog"></div>',
        el: "#medicaid-number-view",
        events: function () {
            return {
        ...
        template: function (serialized_model) {
            var templateHtml = $("#pop-template-medicaid-numbers").html();
                ...
                if (consumerId) {
                    _this.model.postData(consumerId, function (model, response) {
                        if (response.status == "ok") {
                            ...
                            _this.parentView.onPopupMedicaidNumberSave(obj);
                        } else {
                            for (let error of response)
                                Utils.notify(error.errorMessage, "error");
                        }
                    ...
                    _this.isProcessing = false;
                    Utils.notify("Can't add medicaid number data range without selected consumer", "error");
                }
            }                                    
        },
    });
