define(function (require) {
    ...
    return BaseView.extend({
        ...
        template: function (serialized_model) {
            var templateHtml = $("#pop-template-note").html();
        ...
        setDefaultData: function () {
            ...
            if (defaultContact && this.model.get("ContactId") == null && !!defaultContact.ContactId) {
                ...
                this.model.set("TypeFromId", defaultContact.NoteFromTypeId);
            }
        },
        setTypeField: function () {
            ...
            $('#TypeId').on('show.bs.select', function (e) {
                _this.model.resetField($('#TypeId'));
            });

            var _this = this;
            $('#TypeFromId').selectpicker({
                dropupAuto: false
            });
            $('#TypeFromId').on('show.bs.select', function (e) {
                _this.model.resetField($('#TypeFromId'));
            });
        },
        ...
        isEdit: function () {
            var isEdit = this.model.get("ConsumerNoteId") != null;
            return isEdit;
        },       
        onSave: function () {
            var _this = this;
            if (this.model.isValid()) {
                ...
                if ($("#TypeId").val().length > 0) {
                    obj.TypeName = $.trim($("option:selected", "#TypeId").html());
                }
                obj.TypeFromName = "";
                if ($("#TypeFromId").val().length > 0) {
                    obj.TypeFromName = $.trim($("option:selected", "#TypeFromId").html());
                }
                ...
                } else
                {
                    var success = function (responce) {
                        returnObj["ConsumerNoteId"] = responce.id;
